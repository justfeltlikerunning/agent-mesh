...
| `escalation` | Issue flows up the trust hierarchy | Agent-D → Agent-C → Hub-Node |
...
```json
{
  ...
  "context": {
    "originalQuestion": "Count active records in DIST001",
    "responses": [
      {"from": "agent-a", "summary": "1,250", "ts": "..."},
      {"from": "agent-b", "summary": "1,250", "ts": "..."}
    ]
  },
  "body": "Both match. Now count data points for the same district."
}
```
...
```jsonl
{"from":"hub-node","to":"agent-a","type":"request","id":"msg_1",...}
{"from":"hub-node","to":"agent-b","type":"request","id":"msg_2",...}
{"from":"agent-a","to":"hub-node","type":"response","id":"msg_3","correlationId":"msg_1",...}
{"from":"agent-b","to":"hub-node","type":"response","id":"msg_4","correlationId":"msg_2",...}
```
...
📨 conv_abc - Rally to agent-a, agent-b (2m ago)
   "Count active records in DIST001"
   ├── 📊 agent-a: 1,250 (32s)
   ├── 📋 agent-b: 1,250 (45s)
   └── ✅ Complete - Match (0% discrepancy)

📨 conv_def - Escalation: agent-d → agent-c → hub-node (15m ago)
...
- Agent-to-agent direct conversation without Hub-Node routing (keep hub model)
