...
- Generates massive alert floods during outages (early Jan: 40+ duplicate alerts from one incident)
...
|---------|---------|--------|
| **Agents don't execute embedded curl callbacks** | Rally pattern: 6 agents accepted messages, 0 curled back (early Jan) | Complete loss of response |
| **Wrong callback URL/token** | Hub sent Worker-A's callback pointing to Worker-D's IP (early Jan) | Response goes to wrong agent |
| **`deliver: true` is broken** | Mapped hooks with `deliver: true` never produce Slack output (early Jan) | Silent failure |
| **No response confirmation** | HTTP 202 = "accepted" not "processed" | No way to know if agent actually handled it |
| **Alert storms** | 40+ Worker-D alerts for same outage in 6 hours (early Jan) | Alert fatigue, wasted tokens |
...
```json
{
  ...
  "correlationId": null,
  "replyTo": {
    "url": "http://10.0.0.10:18789/hooks/worker-a",
...
```bash
curl -s -X POST http://10.0.0.11:18789/hooks/hub \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer worker1-hook-token" \
  -d '{
    "message": "{\"protocol\":\"mesh/1.0\",\"id\":\"msg_a1b2c3d4\",\"timestamp\":\"2026-01-15T20:45:00Z\",\"from\":\"hub\",\"to\":\"worker-a\",\"type\":\"request\",\"replyTo\":{\"url\":\"http://10.0.0.10:18789/hooks/worker-a\",\"token\":\"hub-hook-token\"},\"priority\":\"normal\",\"ttl\":300,\"payload\":{\"subject\":\"Record count query\",\"body\":\"How many active records are in Region-X?\"}}"
  }'
```
...
{
  "agents": {
    "hub":    { "ip": "10.0.0.10", "port": 18789, "token": "hub-hook-token",       "role": "hub" },
    "worker-1":    { "ip": "10.0.0.11", "port": 18789, "token": "worker1-hook-token",     "role": "worker" },
    "worker-2":   { "ip": "10.0.0.12", "port": 18789, "token": "worker2-hook-token",    "role": "worker" },
    "worker-3": { "ip": "10.0.0.13", "port": 18789, "token": "worker3-hook-token",  "role": "worker" },
    "worker-4": { "ip": "10.0.0.14", "port": 18789, "token": "worker4-hook-token",  "role": "worker" },
    "worker-5":    { "ip": "10.0.0.15", "port": 18789, "token": "worker5-hook-token",     "role": "worker" },
    "worker-6":  { "ip": "10.0.0.16", "port": 18789, "token": "worker6-hook-token",   "role": "worker" }
  }
}
```
...
Implementation: simple JSON state file at `$MESH_HOME/state/circuit-breakers.json`:

```json
{
  "worker-1": { "state": "closed", "failures": 0, "lastFailure": null, "openUntil": null },
  "worker-2": { "state": "open", "failures": 3, "lastFailure": "2026-01-15T20:00:00Z", "openUntil": "2026-01-15T20:01:00Z" }
}
```
...
Messages that fail all retries get written to `$MESH_HOME/state/dead-letters.json`:
...
early Jan: Worker-D fired 40+ alerts for the same network outage over 6 hours. Each was processed as a new event, generating duplicate Slack posts and burning tokens.

### Solution: Incident Tracking

Maintain an incident state file at `$MESH_HOME/state/active-incidents.json`:
  ...
  "incidents": [
    {
      ...
      "firstSeen": "2026-01-15T17:30:00Z",
      "lastSeen": "2026-01-15T23:41:00Z",
      "alertCount": 42,
      "affectedHosts": ["worker-1", "worker-2", "worker-3", "worker-4", "worker-5", "worker-6", "gpu-server", "mac-server"],
      "status": "active",
      "escalated": true,
      "lastNotifiedOperator": "2026-01-15T17:57:00Z"
    }
  ]
}
```
...
5. **Audit log:** Every inter-agent message logged to `$MESH_HOME/logs/mesh-audit.jsonl`:

```jsonl
{"ts":"2026-01-15T20:45:00Z","from":"hub","to":"worker-1","type":"request","id":"msg_a1b2c3d4","subject":"Record count query","status":"sent"}
{"ts":"2026-01-15T20:45:03Z","from":"worker-1","to":"hub","type":"response","id":"msg_e5f6g7h8","correlationId":"msg_a1b2c3d4","status":"received"}
```
...
3. **Create agent registry** at `$MESH_HOME/config/agent-registry.json`
4. **Create state directory** at `$MESH_HOME/state/` with circuit breakers + dead letters + incidents
...
```json
{
  "payload": {
    "attachments": [
      {
        "type": "url",
        "url": "http://10.0.0.10:8890/report.xlsx",
        "filename": "zone-a-report.xlsx",
        "mimeType": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "size": 245760,
        "description": "Region-X data analysis - Q1 2026"
      },
      {
        "type": "inline",
        "encoding": "base64",
      ...
      },
      {
        ...
        "filename": "summary-chart.png",
        "mimeType": "image/png",
        "description": "Data trend chart"
      }
    ]
  }
}
...
mesh-send.sh worker-a request "Analyze this report" \
  --attachment "http://10.0.0.10:8890/report.xlsx|application/vnd.openxmlformats-officedocument.spreadsheetml.sheet|Region-X report"
```
...
```bash
mesh-send.sh worker-a request "Generate Q1 quarterly summary for Region-X" \
  --attachment "http://10.0.0.10:8890/data.csv|text/csv|Well list CSV" \
  --attachment "http://10.0.0.10:8890/chart.png|image/png|Data trend chart"
```
