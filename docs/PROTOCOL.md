...
curl -s -X POST http://10.0.0.11:18789/hooks/hub \
  ...
  -d '{
    "message": "{\"protocol\":\"mesh/1.0\",\"id\":\"msg_a1b2c3d4\",\"timestamp\":\"2026-02-21T20:45:00Z\",\"from\":\"hub\",\"to\":\"worker-a\",\"type\":\"request\",\"replyTo\":{\"url\":\"http://10.0.0.10:18789/hooks/worker-a\",\"token\":\"hub-hook-token\"},\"priority\":\"normal\",\"ttl\":300,\"payload\":{\"subject\":\"Record count query\",\"body\":\"How many active records are in Region A?\"}}"
  }'
```
...
```jsonl
{"ts":"2026-02-21T20:45:00Z","from":"hub","to":"worker-1","type":"request","id":"msg_a1b2c3d4","subject":"Record count query","status":"sent"}
    ...
    "attachments": [
      {
        ...
        "size": 245760,
        "description": "Region A data analysis - Q1 2026"
      },
      {
        "type": "inline",
        "encoding": "base64",
...
mesh-send.sh worker-a request "Analyze this report" \
  --attachment "http://10.0.0.10:8890/report.xlsx|application/vnd.openxmlformats-officedocument.spreadsheetml.sheet|Region A report"
```
...
```bash
mesh-send.sh worker-a request "Generate Q1 quarterly summary for Region A" \
  --attachment "http://10.0.0.10:8890/wells.csv|text/csv|Well list CSV" \
  --attachment "http://10.0.0.10:8890/chart.png|image/png|Production trend chart"
```
