...
#   mesh-converse.sh escalation "Dashboard down after deploy" --agents "agent-d,agent-e,hub-node"
...
This is a collaborative discussion. Share your analysis, findings, or perspective. Your response will be shared with all other participants in subsequent rounds."
    ...
    escalation)
        ...
        if [[ "$AGENTS" == "all" ]]; then
            echo "ERROR: Escalation requires specific agents in order (e.g., --agents agent-d,agent-e,hub-node)" >&2
            exit 1
        fi
