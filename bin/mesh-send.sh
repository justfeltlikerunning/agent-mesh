...
#
# Examples:
#   mesh-send.sh worker-a request "How many assets in Zone-A?"
#   mesh-send.sh worker-a request "Count active records" --wait 120
...
#   mesh-send.sh worker-a response "47 assets" --correlation-id "msg_abc123"
...
send_to_agent() {
    ...
    if [[ "${MESH_TRANSPORT:-auto}" != "http" ]]; then
        # Try hub hub first (single hub of comms), then direct pulse fallback
        local hub_bridge="${SCRIPT_DIR:-$MESH_HOME/bin}/mesh-hub-bridge.sh"
        local bridge_script="${SCRIPT_DIR:-$MESH_HOME/bin}/mesh-pulse-bridge.sh"
        local used_bridge=""
        if [[ -x "$hub_bridge" ]] && "$hub_bridge" "$agent" "$envelope" 2>/dev/null; then
            used_bridge="hub"
        ...
        if [[ -n "$used_bridge" ]]; then
            ...
            echo -e "${GREEN}✓ Sent to ${agent} via hub (${used_bridge})${NC}" >&2
            return 0
        else
            echo -e "${YELLOW}hub + Pulse failed for ${agent} — falling back to HTTP${NC}" >&2
            local queue_script="${SCRIPT_DIR:-$MESH_HOME/bin}/mesh-pulse-queue.sh"
    ...
    while [[ $attempt -lt $retries ]]; do
        ...
        if [[ "$http_code" =~ ^2[0-9][0-9]$ ]]; then
            ...
            # Tap to hub for agent-to-agent visibility
            local tap_script="${SCRIPT_DIR:-$MESH_HOME/bin}/mesh-tap.sh"
